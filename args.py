...
def get_classifier_args():
    ...
    parser.add_argument('--extract_gene_scores',
                        action="store_true",
                        help='Whether to extract and save gene importance scores')
...
def add_train_test_args(parser):
    ...
    parser.add_argument('--pool_ratio',
                    type=float,
                    default=0.5,
                    help='Pooling ratio for TopKPooling in GCNPredictor.')
    parser.add_argument('--pred_mlp_hidden',
                        type=int,
                        nargs='+',
                        default=[64],
                        help='Hidden sizes for the prediction branch MLP (list of ints).')
    parser.add_argument('--gene_mlp_hidden',
                        type=int,
                        nargs='+',
                        default=[64],
                        help='Hidden sizes for the gene selection branch MLP (list of ints).')

    parser.add_argument('--hidden_size',
                        type=int,
                        default=16,
                        help='Number of features in encoder hidden layers.')
    parser.add_argument('--embed_sizes',
                        type=int,
                        default=[768,256,64,16],
                        help='Number of features in encoder hidden layers.')
    parser.add_argument('--expander_output_size',
                        type=int,
                        default=32,
                        help='Number of features in encoder hidden layers.')

    parser.add_argument('--classifier_hidden_size',
                        type=int,
                        default=[32,16],
                        help='Number of features in encoder hidden layers.')
...
def add_common_args(parser):
    ...
    parser.add_argument('--pathway_mode',
                        type=str,
                        default="pairs",
                        help="Pathway process mode: pairs/pathways")
    parser.add_argument('--network_database_path',
                        type=str,
                        default="data/network/network/processed_network.npz",
                        help="Path of network database")
    parser.add_argument('--gs_path',
                        type=str,
                        default="data/network/gene_feature/gene_gs_dict.json",
                        help="Path of gene set feature")

    parser.add_argument('--gene_symbol_file_path',
                        type=str,
                        default="data/Xiao_data/scripts/genes.npz",   
                        help="Gene symbol list for the input data.")

    parser.add_argument('--control_file_path',
                        type=str,
                        default="data/SR006603_WT_5792/filtered_feature_bc_matrix/SR006603_WT_5792_no_headers.csv",
                        help="Path of gene set feature")

    parser.add_argument('--test_file_path',
                        type=str,
                        default="data/SR006603_HOM_5790/filtered_feature_bc_matrix/SR006603_HOM_5790_no_headers.csv",
                        help="Path of gene set feature")
    
    parser.add_argument('--llm_text_embedding_file_path',
                        type=str,
                        default="data/text/gene_text_embs.npy",
                        help="Path of llm generated gene description embedding")

    parser.add_argument('--llm_genesets_embedding_file_path',
                        type=str,
                        default="data/text/geneset_text_embs.npy",
                        help="Path of llm generated genesets description embedding")
