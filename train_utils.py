...
# class LoadExpanderDataset(data.Dataset):
#     def __init__(self,
#                  expression: np.array,
#                  label: np.array):
#         super(LoadClassifierDataset, self).__init__()
...
class CheckpointSaver:
    ...
    def is_reg_best(self, metric_val: float) -> bool:
        if metric_val is None:
            # No metric reported
            return False

        if self.best_reg is None:
            # No checkpoint saved yet
            return True

        return (self.best_reg > metric_val)
    ...
    def save(self,
        ...
        if self.is_reg_best(metric_val):
            # Save the best model
            self.best_reg = metric_val
            best_path = os.path.join(self.save_dir, 'best_reg')
            for name in model_dict.keys():
                shutil.copy(f"{checkpoint_path}{name}.pth.tar", f"{best_path}{name}.pth.tar")

            self._print(f'New reg best reg checkpoint at step {step}...')
